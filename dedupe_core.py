"""Modulo unico di deduplicazione con logging."""
import os
import logging
...
def file_hash(path):
    try:
        ...
        logger.error("Errore calcolo hash %s: %s", path, e)
        return None
...
def deduplica_file(path, db_dir=None):
    ...
    def _run():
        ...
        if drop == path:
            ...
            logger.info("Dedupe %s: duplicato di %s -> RIMOSSO", os.path.basename(path), os.path.basename(keep))
            return False
        removed_others = 0
        for s in others:
            if s != keep:
        ...
        insert_file(db_dir, path, h)
        if removed_others:
            logger.info("Dedupe %s: tenuto, rimossi %d duplicati", os.path.basename(path), removed_others)
        else:
            logger.info("Dedupe %s: già presente (tenuto)", os.path.basename(path))
        return True
        ...
        for path in _iter_media(root):
            ...
            if h in seen:
                ...
                if drop == path:
                    ...
                    removed += 1
                    logger.info("Sweep %s: duplicato -> RIMOSSO", os.path.basename(path))
                else:
                    try:
                        os.remove(seen[h])
                    except Exception:
